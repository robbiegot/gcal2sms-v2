...
const ConfigureReminders = ({ existingDefaults }) => {
    ...
    const [newReminder, setNewReminder] = useState({ time: null, timeUnit: 'hours', text: '', editing: false });
    const [snackbar, setSnackbar] = useState<Pick<AlertProps, 'children' | 'severity'> | null>(null);
    const submitData = () => {
        return useCallback(
            async (method, info: displayReminder) => {
                const data = await fetch(`/api/settings-API/reminders`, {
                    method: method,
    ...
    const processReminderDelete = useCallback(
        async (id) => {
            const response = await mutateDefault("DELETE", id);
    ...
    const handleSaveNewReminder = async () => {
        if (newReminder.time !== null && newReminder.text !== '') {
    ...
    const handleDeleteReminder = async (indexToDelete) => {
        await processReminderDelete(indexToDelete);
                                    ...
                                    <Box sx={{ display: 'flex', marginBottom: '16px' }}>
                                        <Box display="flex" sx={{ minWidth: '13vw', maxWidth: '13vw' }}>
                                            ...
                                            <Select
                                                ...
                                                size="small"
                                                onChange={(e) =>
                                                    setNewReminder({ ...newReminder, timeUnit: e.target.value })
                                                }
                                                sx={{ marginRight: '16px' }}
                                            >
                                                {timeUnits.map((unit) => (
